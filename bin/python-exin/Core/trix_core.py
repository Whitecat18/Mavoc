...
def initiate_hoax_server():
	...
	try:
		...
		try:
			...
			exit(f'[{DEBUG}] HTTP-Payload server failed to start. Port {port} seems to already be in use.\n')
		
		except:
			exit(f'\n[{DEBUG}] HTTP-Payload server failed to start (Unknown error occurred).\n')
		...
		print(f'\rServer Connection Lisening at {GREEN}{Hoaxshell_settings.bind_address}{END}:{ORANGE}{port}{END}\n')
...
class Core_server:
	...
	def initiate(self):
		...
		print(f'\r\n\nPayload Sequer started on {GREEN}{Core_server_settings.bind_address}{END}:{ORANGE}{Core_server_settings.bind_port}{END}')
...
class Netcat_multi_listener:
	...
	def initiate_nc_listener(self):
		...
		print(f'\rNetcat listener started on {GREEN}{Netcat_settings.bind_address}{END}:{ORANGE}{Netcat_settings.bind_port}{END}')
